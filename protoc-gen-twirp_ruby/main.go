...
func (g *generator) generateFile(file *descriptor.FileDescriptorProto) *plugin.CodeGeneratorResponse_File {
	...
	modules := PackageToModules(pkgName)
	for _, m := range modules {
		g.P(fmt.Sprintf("%smodule %s", indent, m))
		indent += 1
	}
...
// Example
//
//	PackageToModules("twitch.twirp.example.helloworld") => ["Twitch", "Twirp", "Example", "Helloworld]
func PackageToModules(pkgName string) []string {
	if pkgName == "" {
		return nil
	}
